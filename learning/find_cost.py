...
cat {conf_file}
conf=`cat {conf_file}`
echo /usr/bin/python {fd_path} --build release64 --overall-time-limit {time_limit} --overall-memory-limit {memory_limit} {domain} {problem} $conf
/usr/bin/python {fd_path} --build release64 --overall-time-limit {time_limit} --overall-memory-limit {memory_limit} {domain} {problem} $conf
"""
...
for i, c in enumerate(CONFS):
    ...
    c_file = open(os.path.join(subdir, CONF_FILE),'w')
    c_file.write(c)
    c_file.close()
